...
class automaticActivity : AppCompatActivity(), OnMapReadyCallback, LocationListener,
    ...
    private var messageFromService :String =""
    ...
    private val mMessageReceiver: BroadcastReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            // Get extra data included in the Intent
             messageFromService = intent.getStringExtra("key")!!
            println("debug: in autoMaticActivity, recieved broadcast message =>${messageFromService}")
...
//        extras.putString(Globals.CLASS_LABEL_KEY, label)
        ...
        println("debug: ok , i will start the Sensor Service")
        startService(mServiceIntent)
        println("debug: ok , this line is after the startService")
    ...
    override fun onLocationChanged(location: Location) {
        ...
        //let's try this, center the camera if the location is chagned
...
//        mMap.addMarker(markerOptions)
    ...
    fun onSaveClicked(view: View){
        ...
        //let's delay this one. maybe 2 seconds
        Handler().postDelayed({
            println("debug: hey this is Handler, 2 seconds delay. how is it going??????")
...
//        Location.distanceBetween(latSfu, lngSfu, latStanley, lngStanley, result)
    ...
    fun insertHistoryIntoDB(latLngsString:String){
        ...
        runHistoryObj.inputType = 2//means automatic
...
//        selectedMonth ="${currentYear} : ${currentMonth} : ${currentDay}"
        ...
        println("debug: before we put this entry intothe DB, the messageFromService from Service is like this =>${messageFromService}")
        // activityType 0 =>Run , 1=>Walk, 2=>Stand
        runHistoryObj.activityType = activityTypeAfterPrediction(messageFromService.toFloat())
        println("debug: ok, after throwing into DB, activityType is ${runHistoryObj.activityType}")
    ...
    fun activityTypeAfterPrediction(value:Float):Int{
        if(value <0.5){
            //standing
            return 2
        }
